...
def insert_history(text):
    cur.execute("INSERT INTO historic VALUES (NULL, '{}')".format(text))
    print("Added to historic")

def delete_history():
    cur.execute("DELETE FROM historic")

def show_history():
    rows = cur.execute("SELECT * FROM historic")
    for row in rows:
        print(row)
...
def iqual():
    if singal.get() == 1:
        ...
        terms_without_errors = brute_terms.replace("x", "*")
        if '÷' in terms_without_errors:
            terms_without_errors = terms_without_errors.replace("÷", "/")

        try:
            result = str(eval(terms_without_errors))
            entry.delete(0, END)
            entry.insert(0, result)
            label_value.set(brute_terms)
            operation.set("IQU")
            print(label_value.get() + " = " + result)
            insert_history(label_value.get() + " = " + result)
            conn.commit()
        except:
...
histbtn = Button(root, text= "historic", fg = "White", bg = "black", command = show_historic)
...
histbtn.grid(row =7, column = 0, columnspan = 4, sticky = W+E, ipady = 2)
root.mainloop()
conn.close()